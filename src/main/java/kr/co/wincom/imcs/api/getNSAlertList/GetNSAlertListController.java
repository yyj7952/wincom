...
@Controller
public class GetNSAlertListController {
	private Log imcsLogger		= LogFactory.getLog("API_Common_getNSAlertList");
	private Log statLogger		= LogFactory.getLog("TLO_getNSAlertList");

	@Autowired
	private GetNSAlertListService getNSAlertListService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	/**
	 * 알람받기목록 가져오기 (lgvod664.pc)
	 * http://localhost:8100/servlets/CommSvl?CMD=getNSAlertList&PARAM=SA_ID=M20110725001|STB_MAC=v201.1072.5001|CONTS_GB=NSC|PAGE_NO=1|PAGE_CNT=10|ORDER_GB=N|QUICK_DIS_YN=Y|
	 */
	public Object getNSAlertList(HttpServletRequest request, HttpServletResponse response , String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSAlertListResultVO resultVO		= new GetNSAlertListResultVO();
		GetNSAlertListRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSAlertListRequestVO(szParam);
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_PRO_ID664) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.API_PRO_ID664 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO	= this.getNSAlertListService.getNSAlertList(requestVO);
		} catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSAlertListResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSAlertListResultVO();
		} finally {
			// 결과 로그 출력
