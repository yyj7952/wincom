...
@Controller
public class GetCopyCacheController {
	private Log imcsLogger =	LogFactory.getLog("API_Common_getCopyCache");
	private Log statLogger =	LogFactory.getLog("TLO_getCopyCache");
	
	@Autowired
	private GetCopyCacheService getCopyCacheService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";
	
	public Object getCopyCache(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetCopyCacheRequestVO requestVO		= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetCopyCacheRequestVO(szParam);
			saId		= "";
			stbMac		= "";
		} catch (Exception e) {
			...
			String msg = " svc[" + ImcsConstants.API_COPYCACHE_ID001 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			// 입입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_COPYCACHE_ID001) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			requestVO.setPid(pId);
			this.getCopyCacheService.getCopyCache(requestVO);
			
		} catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetCopyCacheResultVO();
