...
@Controller
public class GetNSWatchListController {
	private Log imcsLogger	= LogFactory.getLog("API_Common_getNSWatchList");
	private Log statLogger	= LogFactory.getLog("TLO_getNSWatchList");
	
	@Autowired
	private GetNSWatchListService GetNSWatchListService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	public Object getNSWatchList(HttpServletRequest request, HttpServletResponse response
			, String szParam, String szStat) throws Exception
	{
		...
		GetNSWatchListRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSWatchListRequestVO(szParam);
			saId		= requestVO.getSaId();
			stbMac		= requestVO.getMacAddr();
		} catch (Exception e) {
			resultVO.setResultCode("31000000");
			IMCSLog imcsLog = new IMCSLog(imcsLogger, saId, stbMac, pId);
			
			// 인입로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.NSAPI_PRO_ID710) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.NSAPI_PRO_ID710 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
			imcsLog.serviceLog(msg, methodName, methodLine);
			
			return "1|REQUEST PARAMETER ERROR|0||||" + ImcsConstants.ROWSEP; //파라미터오류리턴
		...
		try {
		    ...
		    resultVO = this.GetNSWatchListService.getNSWatchList(requestVO);
		}  catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSWatchListResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSWatchListResultVO();
		} finally {
			// 결과 로그 출력
