...
@Controller
public class GetNSChPGM2Controller {

	private Log imcsLogger	= LogFactory.getLog("API_Common_getNSChPGM2");
	private Log statLogger	= LogFactory.getLog("TLO_getNSChPGM2");

	@Autowired
	private GetNSChPGM2Service GetNSChPGM2Service;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	public Object getNSChPGM2(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSChPGM2ResultVO resultVO		= new GetNSChPGM2ResultVO();
		GetNSChPGM2RequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSChPGM2RequestVO(szParam);
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.NSAPI_PRO_ID021) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.NSAPI_PRO_ID021 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO	= this.GetNSChPGM2Service.getNSChPGM2(requestVO);
		} catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSChPGM2ResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSChPGM2ResultVO();
		} finally {
			// 결과 로그 출력
