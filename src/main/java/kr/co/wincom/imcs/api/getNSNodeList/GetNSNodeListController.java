...
@Controller
public class GetNSNodeListController {

	private Log imcsLogger	= LogFactory.getLog("API_Common_getNSNodeList");
	private Log statLogger	= LogFactory.getLog("TLO_getNSNodeList");

	@Autowired
	private GetNSNodeListService GetNSNodeListService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	public Object getNSNodeList(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSNodeListResultVO resultVO		= new GetNSNodeListResultVO();
		GetNSNodeListRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSNodeListRequestVO(szParam);
			...
			String resultHeader  = String.format("%s|%s|", "1", "INPUT PARAM ERROR");
	        resultVO.setResultHeader(resultHeader);
	        return resultVO;
		}
		...
		try {
			...
			resultVO	= this.GetNSNodeListService.getNSNodeList(requestVO);
		} catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSNodeListResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSNodeListResultVO();
		} finally {
			// 결과 로그 출력
