...
@Controller
public class GetNSKidsEStudyController
{
	private final static String API_LOG_NAME = "000/getNSKidsEStudy";
	
	private Log imcsLogger		= LogFactory.getLog("API_Common_getNSKidsEStudy");
	private Log statLogger		= LogFactory.getLog("TLO_getNSKidsEStudy");
	
	@Autowired
	private GetNSKidsEStudyService getNSKidsEStudyService;
	
	public Object getNSKidsEStudy(HttpServletRequest request, HttpServletResponse response , String szParam, String szStat) throws Exception
	{
		// 프로세스 정보
		...
		GetNSKidsEStudyRequestVO requestVO = null;
		...
		try
		{
			requestVO	= new GetNSKidsEStudyRequestVO(szParam);
		...
		try
		{
			...
			resultVO = this.getNSKidsEStudyService.getNSKidsEStudy(requestVO);
		}
		catch(ImcsException ie)
		{
			if(resultVO == null)		resultVO = new GetNSKidsEStudyResultVO();
		}
		catch(Exception e)
		{
			if(resultVO == null)		resultVO = new GetNSKidsEStudyResultVO();
