...
@Controller
public class GetNSSubCountController
{
	private final static String API_LOG_NAME = "000/getNSSubCount";
	
	private Log imcsLogger		= LogFactory.getLog("API_Common_getNSSubCount");
	private Log statLogger		= LogFactory.getLog("TLO_getNSSubCount");
	
	@Autowired
	private GetNSSubCountService getNSSubCountService;
	
	public Object getNSSubCount(HttpServletRequest request, HttpServletResponse response , String szParam, String szStat) throws Exception
	{
		// 프로세스 정보
		...
		GetNSSubCountRequestVO requestVO = null;
		...
		try
		{
			requestVO	= new GetNSSubCountRequestVO(szParam);
		...
		try
		{
			...
			resultVO = this.getNSSubCountService.getNSSubCount(requestVO);
		}
		catch(ImcsException ie)
		{
			if(resultVO == null)		resultVO = new GetNSSubCountResultVO();
		}
		catch(Exception e)
		{
			if(resultVO == null)		resultVO = new GetNSSubCountResultVO();
