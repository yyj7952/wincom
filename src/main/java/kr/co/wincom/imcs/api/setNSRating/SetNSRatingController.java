...
@Controller
public class SetNSRatingController {
	private Log imcsLogger = LogFactory.getLog("API_Common_setNSRating");
	private Log statLogger = LogFactory.getLog("TLO_setNSRating");
	
	@Autowired
	private SetNSRatingService SetNSRatingService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";
	
	public Object setNSRating(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception	{
		// 프로세스 정보
		...
		SetNSRatingRequestVO requestVO		= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new SetNSRatingRequestVO(szParam);
			...
			// 입입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_PRO_ID999) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.API_PRO_ID999 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO = this.SetNSRatingService.setNSRating(requestVO);
		}  catch(ImcsException ie) {
			if( resultVO == null ) resultVO = new SetNSRatingResultVO();	
