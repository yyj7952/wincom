...
@Controller
public class AuthorizeVViewController {
	private Log imcsLogger		= LogFactory.getLog("API_Common_authorizeVView");
	private Log statLogger		= LogFactory.getLog("TLO_authorizeVView");

	@Autowired
	private AuthorizeVViewService AuthorizeVViewService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	public Object authorizeVView(HttpServletRequest request, HttpServletResponse response
		...
		AuthorizeVViewResultVO resultVO		= new AuthorizeVViewResultVO();
		AuthorizeVViewRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new AuthorizeVViewRequestVO(szParam);
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_PRO_ID177) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.API_PRO_ID177 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO	= this.AuthorizeVViewService.authorizeVView(requestVO);
		} catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new AuthorizeVViewResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new AuthorizeVViewResultVO(); 
		} finally {
			// 결과 로그 출력
