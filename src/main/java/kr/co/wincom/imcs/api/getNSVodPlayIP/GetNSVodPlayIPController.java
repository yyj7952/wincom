...
@Controller
public class GetNSVodPlayIPController {

	private Log imcsLogger	= LogFactory.getLog("API_Common_getNSVodPlayIP");
	private Log statLogger	= LogFactory.getLog("TLO_getNSVodPlayIP");

	@Autowired
	private GetNSVodPlayIPService GetNSVodPlayIPService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	public Object getNSVodPlayIP(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSVodPlayIPResultVO resultVO		= new GetNSVodPlayIPResultVO();
		GetNSVodPlayIPRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSVodPlayIPRequestVO(szParam);
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.NSAPI_PRO_ID070) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.NSAPI_PRO_ID070 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO	= this.GetNSVodPlayIPService.getNSVodPlayIP(requestVO);
		} catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSVodPlayIPResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSVodPlayIPResultVO();
		} finally {
			// 결과 로그 출력
