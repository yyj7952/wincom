...
@Controller
public class GetNSCHListsController {
	private Log imcsLogger	= LogFactory.getLog("API_Common_getNSCHLists");
	private Log statLogger	= LogFactory.getLog("TLO_getNSCHLists");
	
	@Autowired
	private GetNSCHListsService GetNSCHListsService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	public Object getNSCHLists(HttpServletRequest request, HttpServletResponse response
			, String szParam, String szStat) throws Exception
	{
		...
		GetNSCHListsResultVO resultVO	= new GetNSCHListsResultVO();
		GetNSCHListsRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSCHListsRequestVO(szParam);
			...
			// 인입로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_PRO_ID035) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.API_PRO_ID035 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
		    ...
		    resultVO = this.GetNSCHListsService.getNSCHLists(requestVO);
		}  catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSCHListsResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSCHListsResultVO();
		} finally {
			// 결과 로그 출력
