...
@Controller
public class RmNSAllAlertController {
	private Log imcsLogger		= LogFactory.getLog("API_Common_rmNSAllAlert");
	private Log statLogger		= LogFactory.getLog("TLO_rmNSAllAlert");

	@Autowired
	private RmNSAllAlertService		rmNSAllAlertService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	/**
	 * 찜목록 전체 삭제 (lgvod663.pc)
	 */
	public Object rmNSAllAlert(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		RmNSAllAlertResultVO resultVO		= new RmNSAllAlertResultVO();
		RmNSAllAlertRequestVO requestVO		= null;
		String saId		= "";
		String stbMac	= "";
		
		// 변수 초기화
		try {
			requestVO	= new RmNSAllAlertRequestVO(szParam);
			...
			String msg = " svc[" + ImcsConstants.API_PRO_ID663 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_PRO_ID663) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			requestVO.setPid(pId);
			resultVO = this.rmNSAllAlertService.rmNSAllAlert(requestVO);
		} catch(ImcsException ie) {
			if( ie.getList() != null )	resultVO = (RmNSAllAlertResultVO) ie.getList();
			else						resultVO = new RmNSAllAlertResultVO();
			
			resultVO.setFlag(ie.getFlag());
			resultVO.setErrMsg(ie.getMessage());
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new RmNSAllAlertResultVO();
			resultVO.setFlag("1");
			resultVO.setErrMsg("알림받기 삭제가 실패하였습니다.");
		} finally {
			// 결과 로그 출력
