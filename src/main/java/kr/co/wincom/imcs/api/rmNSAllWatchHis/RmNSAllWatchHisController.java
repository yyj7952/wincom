...
@Controller
public class RmNSAllWatchHisController {
	private Log imcsLogger	= LogFactory.getLog("API_Common_rmNSAllWatchHis");
	private Log statLogger	= LogFactory.getLog("TLO_rmNSAllWatchHis");

	@Autowired
	private RmNSAllWatchHisService RmNSAllWatchHisService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	public Object rmNSAllWatchHis(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		RmNSAllWatchHisResultVO resultVO	= new RmNSAllWatchHisResultVO();
		RmNSAllWatchHisRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new RmNSAllWatchHisRequestVO(szParam);
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_PRO_ID358) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.API_PRO_ID358 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO	= this.RmNSAllWatchHisService.rmNSAllWatchHis(requestVO);
		} catch(ImcsException ie) {
			if( ie.getList() != null )	resultVO = (RmNSAllWatchHisResultVO) ie.getList();
			else						resultVO = new RmNSAllWatchHisResultVO();
			resultVO.setFlag(ie.getFlag());
			resultVO.setErrMsg(ie.getMessage());
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new RmNSAllWatchHisResultVO();
			
			resultVO.setFlag("1");
			resultVO.setErrMsg("시청목록 삭제가 실패하였습니다.");
		} finally {
			// 결과 로그 출력
