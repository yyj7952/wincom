package kr.co.wincom.imcs.api.buyNSConts;

import java.lang.management.ManagementFactory;

import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import kr.co.wincom.imcs.common.ImcsConstants;
import kr.co.wincom.imcs.common.util.IMCSLog;
import kr.co.wincom.imcs.common.util.StringUtil;
import kr.co.wincom.imcs.common.vo.StatVO;
import kr.co.wincom.imcs.handler.ImcsException;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Controller;

@Controller
public class BuyNSContsController {
		
	private Log imcsLogger = LogFactory.getLog("API_Common_buyNSConts");
	private Log statLogger = LogFactory.getLog("TLO_buyNSConts");
	
	@Autowired
	private BuyNSContsService buyNSContsService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";
	
	public Object buyNSConts(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		StackTraceElement oStackTrace = Thread.currentThread().getStackTrace()[1];
		String pName	= ManagementFactory.getRuntimeMXBean().getName();		// 프로세스네임 
		String pId		= String.valueOf(Thread.currentThread().getId());				// 프로세스ID
		String methodName	= oStackTrace.getMethodName();						// 메소드명
		String methodLine	= String.valueOf(oStackTrace.getLineNumber() - 3);	// 메소드라인
		
		BuyNSContsResultVO resultVO = new BuyNSContsResultVO();
		BuyNSContsRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		long tp_start = System.currentTimeMillis();
		
		
		// 변수 초기화
		try {
			requestVO	= new BuyNSContsRequestVO(szParam);
			saId		= requestVO.getSaId();
			stbMac		= requestVO.getStbMac();
		} catch (Exception e) {
			resultVO.setResultCode("31000000");
			IMCSLog imcsLog = new IMCSLog(imcsLogger, saId, stbMac, pId);
			
			// 입수로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.NSAPI_PRO_ID010) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.NSAPI_PRO_ID010 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
			imcsLog.serviceLog(msg, methodName, methodLine);
			
			return "";
			//return "1|정상적인 INPUT PARAM이 아닙니다.|||" + ImcsConstants.ROWSEP; //파라미터오류리턴 (2019.10.21 수정)
		}
		

		// 통계로그 정보
		StatVO statVO	= new StatVO();
		try{
			statVO	= new StatVO(szStat);
			statVO.setApiNm(methodName);
			statVO.setSaId(saId);
			statVO.setStbMac(stbMac);
		} catch (Exception e) {
			resultVO.setResultCode("31000000");
			throw new ImcsException();
		}
		
		// 서비스로그 초기화
		IMCSLog imcsLog = new IMCSLog(imcsLogger, saId, stbMac, pId);			
		requestVO.setTp_start(tp_start);
		
		try {
			// 입수로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.NSAPI_PRO_ID010) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			requestVO.setPid(pId);
			resultVO = this.buyNSContsService.buyNSConts(requestVO);
		} catch(ImcsException ie) {
			if( ie.getList() != null )	resultVO	= (BuyNSContsResultVO) ie.getList();
			else						resultVO = new BuyNSContsResultVO();
			
			resultVO.setFlag(ie.getFlag());			
			resultVO.setErrCode(ie.getErrorCode());
			resultVO.setErrMsg(ie.getMessage());
			
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new BuyNSContsResultVO();
			resultVO.setFlag("1");
			resultVO.setErrCode("99"); 
		} finally {
			// 결과 로그 출력
			long tp_end = System.currentTimeMillis();
			imcsLog.timeLog("[" + methodName + "] tx_time", String.valueOf(tp_end - tp_start), methodName, methodLine);
		 
			IMCSLog statLog = new IMCSLog(statLogger);
			statVO.setSvcName(requestVO.getAppType());
			statVO.setContentsId(requestVO.getAlbumId());
			statVO.setContentsName(requestVO.getAlbumName());
			statVO.setProductId(resultVO.getProductId());
			statVO.setProductName(resultVO.getProductName());
			statVO.setProductPrice(resultVO.getProductPrice());
			statVO.setBuyingType(requestVO.getBuyingType());
			statVO.setBuyAmt(requestVO.getBuyingPrice());
			statVO.setAlwncecharge(requestVO.getAlwnceCharge());
			statVO.setBuyingGb(requestVO.getBuyingGb());
			statVO.setApiType("B");
			
			if("A".equals(requestVO.getBuyingType().toUpperCase()))
				statVO.setBalace(requestVO.getBalace());
			else 
				statVO.setBalace(requestVO.getBuyingPrice());
			
			
			switch (requestVO.getBuyTypeFlag()) {
			case "2":
			case "3":
				statVO.setDfBuyYn("Y");
				statVO.setDfPrice(requestVO.getSuggestedDatafreePrice());
				
				if( "A".equals( requestVO.getBuyingType().toUpperCase() ) ){
					statVO.setDfBalace(requestVO.getSuggestedDatafreeApprovalPrice());
				}else{
					statVO.setDfBalace(requestVO.getSuggestedDatafreePrice());
				}
				
				break;

			case "1":
			default:
				statVO.setDfBuyYn("N");
				break;
			}
			
			statVO.setBuyingDate(resultVO.getBuyingDate());
			statVO.setResultCode(StringUtil.replaceNull(resultVO.getResultCode(), statVO.getResultCode()));
			statLog.statLog(statVO.toString());
		}
		return resultVO;
	}
}
