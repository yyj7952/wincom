...
@Controller
public class GetNSVSIController {
	private Log imcsLogger	= LogFactory.getLog("API_Common_getNSVSI");
	private Log statLogger	= LogFactory.getLog("TLO_getNSVSI");

	@Autowired
	private GetNSVSIService GetNSVSIService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";
	public Object getNSVSI(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSVSIResultVO resultVO	= new GetNSVSIResultVO();
		GetNSVSIRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSVSIRequestVO(szParam);
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_PRO_ID032) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.API_PRO_ID032 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO	= this.GetNSVSIService.getNSVSI(requestVO);
		}  catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSVSIResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSVSIResultVO();
		} finally {
			// 결과 로그 출력
