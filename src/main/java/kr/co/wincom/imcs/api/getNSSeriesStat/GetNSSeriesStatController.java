...
@Controller
public class GetNSSeriesStatController {
	private Log imcsLogger		= LogFactory.getLog("API_Common_getNSSeriesStat");
	private Log statLogger		= LogFactory.getLog("TLO_getNSSeriesStat");
	
	@Autowired
	private GetNSSeriesStatService getNSSeriesStatService;
	
	
	public Object getNSSeriesStat(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		getNSSeriesStatResultVO resultVO		= new getNSSeriesStatResultVO();
		GetNSSeriesStatRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSSeriesStatRequestVO(szParam);
			...
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.NSAPI_PRO_ID230) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.NSAPI_PRO_ID230 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO	= this.getNSSeriesStatService.getNSSeriesStat(requestVO);
		} catch(ImcsException ie) {
			if(resultVO == null)		resultVO = new getNSSeriesStatResultVO();
		} catch(Exception e) {
			if(resultVO == null)		resultVO = new getNSSeriesStatResultVO();
		} finally {
			// 결과 로그 출력
