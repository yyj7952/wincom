...
@Controller
public class GetNSVisitDtlController {
	private Log imcsLogger	= LogFactory.getLog("API_Common_getNSVisitDtl");
	private Log statLogger	= LogFactory.getLog("TLO_getNSVisitDtl");

	@Autowired
	private GetNSVisitDtlService GetNSContDtlService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	public Object getNSVisitDtl(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSVisitDtlResultVO resultVO		= new GetNSVisitDtlResultVO();
		GetNSVisitDtlRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSVisitDtlRequestVO(szParam);
			...
			String msg = " svc[" + ImcsConstants.API_PRO_ID984 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_PRO_ID984) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			requestVO.setPid(pId);
			resultVO	= this.GetNSContDtlService.getNSVisitDtl(requestVO);
		} catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSVisitDtlResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSVisitDtlResultVO(); 
		} finally {
			// 결과 로그 출력
