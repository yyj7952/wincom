...
@Controller
public class ChkBuyNSPGController {
	private Log imcsLogger	= LogFactory.getLog("API_Common_chkBuyNSPG");
	
	@Autowired
	private ChkBuyNSPGService ChkBuyNSPGService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	public Object chkBuyNSPG(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		ChkBuyNSPGRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new ChkBuyNSPGRequestVO(szParam);
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_PRO_ID258) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.API_PRO_ID258 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
			imcsLog.serviceLog(msg, methodName, methodLine);
			
			return "1|INPUT PARAM ERROR|||" + ImcsConstants.ROWSEP; //파라미터오류리턴 (2019.10.21 수정)
		}
		...
		try {
			...
			resultVO = this.ChkBuyNSPGService.chkBuyNSPG(requestVO);
		} catch(ImcsException ie) {
			if( ie.getList() != null )	resultVO	= (ChkBuyNSPGResultVO) ie.getList();
			else						resultVO = new ChkBuyNSPGResultVO();
			
			resultVO.setFlag(ie.getFlag());
			resultVO.setErrCode(ie.getErrorCode());
			resultVO.setErrMsg(ie.getMessage());			
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new ChkBuyNSPGResultVO();
			resultVO.setFlag("1");
