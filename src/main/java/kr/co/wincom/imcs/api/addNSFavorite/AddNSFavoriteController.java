...
@Controller
public class AddNSFavoriteController {
	private Log imcsLogger		= LogFactory.getLog("API_Common_addNSFavorite");
	private Log statLogger		= LogFactory.getLog("TLO_addNSFavorite");

	@Autowired
	private AddNSFavoriteService	addNSFavoriteService;
	 ...
	 * 찜목록 등록 (lgvod379.pc)
	 */
	public Object addNSFavorite(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		...
		AddNSFavoriteResultVO resultVO		= new AddNSFavoriteResultVO();
		AddNSFavoriteRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new AddNSFavoriteRequestVO(szParam);
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_PRO_ID379) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.API_PRO_ID379 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO	= this.addNSFavoriteService.addNSFavorite(requestVO);
		} catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new AddNSFavoriteResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new AddNSFavoriteResultVO();
		} finally {
			// 결과 로그 출력
