...
@Controller
public class GetNSMapInfoController
{
	private final static String API_LOG_NAME = "000/getNSMapInfo";
	
	private Log imcsLogger		= LogFactory.getLog("API_Common_getNSMapInfo");
	private Log statLogger		= LogFactory.getLog("TLO_getNSMapInfo");
	
	@Autowired
	private GetNSMapInfoService getNSMapInfoService;
	
	public Object getNSMapInfo(HttpServletRequest request, HttpServletResponse response , String szParam, String szStat) throws Exception
	{
		// 프로세스 정보
		...
		GetNSMapInfoRequestVO requestVO = null;
		...
		try
		{
			requestVO	= new GetNSMapInfoRequestVO(szParam);
		...
		try
		{
			...
			resultVO = this.getNSMapInfoService.getNSMapInfo(requestVO);
		}
		catch(ImcsException ie)
		{
			if(resultVO == null)		resultVO = new GetNSMapInfoResultVO();
		}
		catch(Exception e)
		{
			if(resultVO == null)		resultVO = new GetNSMapInfoResultVO();
