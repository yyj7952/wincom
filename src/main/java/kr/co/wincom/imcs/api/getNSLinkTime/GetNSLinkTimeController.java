...
@Controller
public class GetNSLinkTimeController {
	private Log imcsLogger	= LogFactory.getLog("API_Common_getNSLinkTime");
	private Log statLogger	= LogFactory.getLog("TLO_getNSLinkTime");

	@Autowired
	private GetNSLinkTimeService GetNSLinkTimeService;

	public Object getNSLinkTime(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSLinkTimeResultVO resultVO		= new GetNSLinkTimeResultVO();
		GetNSLinkTimeRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSLinkTimeRequestVO(szParam);
			...
			// 인입로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.NSAPI_GETNSLINKTIME) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.NSAPI_GETNSLINKTIME + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO = this.GetNSLinkTimeService.getNSLinkTime(requestVO);
		} catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSLinkTimeResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSLinkTimeResultVO();
		} finally {
			// 결과 로그 출력
