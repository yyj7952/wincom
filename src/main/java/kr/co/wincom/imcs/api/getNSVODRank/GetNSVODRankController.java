...
@Controller
public class GetNSVODRankController {
	private Log imcsLogger	= LogFactory.getLog("API_Common_getNSVODRank");
	private Log statLogger	= LogFactory.getLog("TLO_getNSVODRank");

	@Autowired
	private GetNSVODRankService GetNSVODRankService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	public Object getNSVODRank(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSVODRankResultVO resultVO	= new GetNSVODRankResultVO();
		GetNSVODRankRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSVODRankRequestVO(szParam);
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_PRO_ID993) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.API_PRO_ID993 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO	= this.GetNSVODRankService.getNSVODRank(requestVO);
		}  catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSVODRankResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSVODRankResultVO();
		} finally {
			// 결과 로그 출력
