...
@Controller
public class UseNSPresentController {
	private Log imcsLogger =	LogFactory.getLog("API_Common_useNSPresent");
	private Log statLogger =	LogFactory.getLog("TLO_useNSPresent");
	
	@Autowired
	private UseNSPresentService UseNSPresentService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";
	
	public Object useNSPresent(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		UseNSPresentRequestVO requestVO		= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new UseNSPresentRequestVO(szParam);
			...
			// 입입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_PRO_ID314) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.API_PRO_ID314 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO	= this.UseNSPresentService.useNSPresent(requestVO);
		} catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new UseNSPresentResultVO();
