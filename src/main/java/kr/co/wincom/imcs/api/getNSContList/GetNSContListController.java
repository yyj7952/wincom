...
@Controller
public class GetNSContListController {
	private Log imcsLogger		= LogFactory.getLog("API_Common_getNSContList");
	private Log statLogger		= LogFactory.getLog("TLO_getNSContList");

	@Autowired
	private GetNSContListService		getNSContListService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	public Object getNSContList(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSContListResultVO resultVO		= new GetNSContListResultVO();
		GetNSContListRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		
		// 변수 초기화
		try {
			requestVO	= new GetNSContListRequestVO(szParam);
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_PRO_ID995) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.API_PRO_ID995 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO	= getNSContListService.getNSContList(requestVO);
		} catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSContListResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSContListResultVO();
		} finally {
			// 결과 로그 출력
			...
			statVO.setScatId(requestVO.getCatId());
