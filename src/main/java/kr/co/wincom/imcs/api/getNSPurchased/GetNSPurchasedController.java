...
@Controller
public class GetNSPurchasedController {
	private Log imcsLogger		= LogFactory.getLog("API_Common_getNSPurchased");
	private Log statLogger		= LogFactory.getLog("TLO_getNSPurchased");
	
	@Autowired
	private GetNSPurchasedService		getNSPurchasedService;
	 ...
	 */
	public Object getNSPurchased(HttpServletRequest request, HttpServletResponse response , String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSPurchasedResultVO resultVO		= new GetNSPurchasedResultVO();
		GetNSPurchasedRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		
		// 변수 초기화
		try {
			requestVO	= new GetNSPurchasedRequestVO(szParam);
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_PRO_ID119) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.API_PRO_ID119 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
			imcsLog.serviceLog(msg, methodName, methodLine);
			msg = " svc[" + ImcsConstants.API_PRO_ID119 + "] sts[error] msg[정상적인 INPUT PARAM이 아닙니다.]";
			imcsLog.serviceLog(msg, methodName, methodLine);
		...
		try {
			...
			resultVO	= this.getNSPurchasedService.getNSPurchased(requestVO);
		} catch(ImcsException ie) {
			if(resultVO == null)		resultVO = new GetNSPurchasedResultVO();
		} catch(Exception e) {
			if(resultVO == null)		resultVO = new GetNSPurchasedResultVO();
		} finally {
			// 결과 로그 출력
