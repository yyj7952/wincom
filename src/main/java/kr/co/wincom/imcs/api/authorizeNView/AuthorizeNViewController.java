...
@Controller
public class AuthorizeNViewController {
	private Log imcsLogger		= LogFactory.getLog("API_Common_authorizeNView");
	private Log statLogger		= LogFactory.getLog("TLO_authorizeNView");

	@Autowired
	private AuthorizeNViewService AuthorizeNViewService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	public Object authorizeNView(HttpServletRequest request, HttpServletResponse response
		...
		AuthorizeNViewResultVO resultVO		= new AuthorizeNViewResultVO();
		AuthorizeNViewRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new AuthorizeNViewRequestVO(szParam);
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_PRO_ID178) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.API_PRO_ID178 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO	= this.AuthorizeNViewService.authorizeNView(requestVO);
		} catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new AuthorizeNViewResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new AuthorizeNViewResultVO(); 
		} finally {
			// 결과 로그 출력
			...
			statVO.setSvcName(requestVO.getApplType());
			statVO.setApiType("V");
