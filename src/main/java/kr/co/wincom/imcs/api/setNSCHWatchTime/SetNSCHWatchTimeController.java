...
@Controller
public class SetNSCHWatchTimeController {
		
	private Log imcsLogger = LogFactory.getLog("API_Common_setNSCHWatchTime");
	private Log statLogger = LogFactory.getLog("TLO_setNSCHWatchTime");
	
	@Autowired
	private SetNSCHWatchTimeService SetNSCHWatchTimeService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";
	
	public Object setNSCHWatchTime(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		SetNSCHWatchTimeRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO 	= new SetNSCHWatchTimeRequestVO(szParam);
			...
			// 인입로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.NSAPI_SetNSCHWatchTime) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.NSAPI_SetNSCHWatchTime + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO = this.SetNSCHWatchTimeService.setNSCHWatchTime(requestVO);
		}  catch(ImcsException ie) {
			if( ie != null)				resultVO = (SetNSCHWatchTimeResultVO) ie.getList();
			if( resultVO == null )		resultVO = new SetNSCHWatchTimeResultVO();
