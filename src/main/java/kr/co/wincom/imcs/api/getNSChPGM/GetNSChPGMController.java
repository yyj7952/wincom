...
@Controller
public class GetNSChPGMController {

	private Log imcsLogger	= LogFactory.getLog("API_Common_getNSChPGM");
	private Log statLogger	= LogFactory.getLog("TLO_getNSChPGM");

	@Autowired
	private GetNSChPGMService GetNSChPGMService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	public Object getNSChPGM(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSChPGMResultVO resultVO		= new GetNSChPGMResultVO();
		GetNSChPGMRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSChPGMRequestVO(szParam);
		...
		try {
			...
			resultVO	= this.GetNSChPGMService.getNSChPGM(requestVO);
		} catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSChPGMResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSChPGMResultVO();
		} finally {
			// 결과 로그 출력
