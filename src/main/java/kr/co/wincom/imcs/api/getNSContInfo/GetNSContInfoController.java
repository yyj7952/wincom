...
@Controller
public class GetNSContInfoController {
	private Log imcsLogger	= LogFactory.getLog("API_Common_getNSContInfo");
	private Log statLogger	= LogFactory.getLog("TLO_getNSContInfo");
	
	@Autowired
	private GetNSContInfoService GetNSContInfoService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	public Object getNSContInfo (HttpServletRequest request, HttpServletResponse response
			, String szParam, String szStat) throws Exception
	{
		...
		GetNSContInfoResultVO resultVO	= new GetNSContInfoResultVO();
		GetNSContInfoRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSContInfoRequestVO(szParam);
			...
			// 인입로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.NSAPI_PRO_ID220) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.NSAPI_PRO_ID220 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
		    ...
		    resultVO = this.GetNSContInfoService.getNSContInfo(requestVO);
		}  catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSContInfoResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSContInfoResultVO();
		} finally {
			// 결과 로그 출력
