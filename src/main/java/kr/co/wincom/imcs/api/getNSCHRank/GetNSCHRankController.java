...
@Controller
public class GetNSCHRankController {
	private Log imcsLogger	= LogFactory.getLog("API_Common_getNSCHRank");
	private Log statLogger	= LogFactory.getLog("TLO_getNSCHRank");

	@Autowired
	private GetNSCHRankService GetNSCHRankService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	public Object getNSCHRank(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSCHRankResultVO resultVO	= new GetNSCHRankResultVO();
		GetNSCHRankRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSCHRankRequestVO(szParam);
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_PRO_ID649) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.API_PRO_ID649 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
		    ...
		    resultVO		= this.GetNSCHRankService.getNSCHRank(requestVO);
		} catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSCHRankResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSCHRankResultVO();
		} finally {
			// 결과 로그 출력
