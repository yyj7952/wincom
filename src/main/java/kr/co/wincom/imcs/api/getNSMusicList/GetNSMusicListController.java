...
@Controller
public class GetNSMusicListController {
	private Log imcsLogger	= LogFactory.getLog("API_Common_getNSMusicList");
	private Log statLogger	= LogFactory.getLog("TLO_getNSMusicList");
	
	@Autowired
	private GetNSMusicListService GetNSMusicListService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	public Object getNSMusicList(HttpServletRequest request, HttpServletResponse response
			, String szParam, String szStat) throws Exception
	{
		...
		GetNSMusicListResultVO resultVO	= new GetNSMusicListResultVO();
		GetNSMusicListRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSMusicListRequestVO(szParam);
			...
			// 인입로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.NSAPI_PRO_ID030) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.NSAPI_PRO_ID030 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
		    ...
		    resultVO = this.GetNSMusicListService.getNSMusicList(requestVO);
		}  catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSMusicListResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSMusicListResultVO();
		} finally {
			// 결과 로그 출력
