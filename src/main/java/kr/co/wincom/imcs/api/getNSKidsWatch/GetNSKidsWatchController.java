...
@Controller
public class GetNSKidsWatchController
{
	private final static String API_LOG_NAME = "000/getNSKidsWatch";
	
	private Log imcsLogger		= LogFactory.getLog("API_Common_getNSKidsWatch");
	private Log statLogger		= LogFactory.getLog("TLO_getNSKidsWatch");
	
	@Autowired
	private GetNSKidsWatchService getNSKidsWatchService;
	
	public Object getNSKidsWatch(HttpServletRequest request, HttpServletResponse response , String szParam, String szStat) throws Exception
	{
		// 프로세스 정보
		...
		GetNSKidsWatchRequestVO requestVO = null;
		...
		try
		{
			requestVO	= new GetNSKidsWatchRequestVO(szParam);
		...
		try
		{
			...
			resultVO = this.getNSKidsWatchService.getNSKidsWatch(requestVO);
		}
		catch(ImcsException ie)
		{
			if(resultVO == null)		resultVO = new GetNSKidsWatchResultVO();
		}
		catch(Exception e)
		{
			if(resultVO == null)		resultVO = new GetNSKidsWatchResultVO();
