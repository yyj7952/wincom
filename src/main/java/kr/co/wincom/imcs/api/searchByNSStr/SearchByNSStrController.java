...
@Controller
public class SearchByNSStrController {
	private Log imcsLogger =	LogFactory.getLog("API_Common_searchByNSStr");
	private Log statLogger =	LogFactory.getLog("TLO_searchByNSStr");
	
	@Autowired
	private SearchByNSStrService SearchByNSStrService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";
	
	public Object searchByNSStr(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		SearchByNSStrResultVO resultVO		= new SearchByNSStrResultVO();
		SearchByNSStrRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new SearchByNSStrRequestVO(szParam);
			...
			// 입입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_PRO_ID939) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.API_PRO_ID939 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO	= this.SearchByNSStrService.searchByNSStr(requestVO);
		} catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new SearchByNSStrResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new SearchByNSStrResultVO();
		} finally {
			// 결과 로그 출력
