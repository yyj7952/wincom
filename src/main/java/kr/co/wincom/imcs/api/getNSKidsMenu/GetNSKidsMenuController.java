...
@Controller
public class GetNSKidsMenuController
{
	private final static String API_LOG_NAME = "000/getNSKidsMenu";
	
	private Log imcsLogger		= LogFactory.getLog("API_Common_getNSKidsMenu");
	private Log statLogger		= LogFactory.getLog("TLO_getNSKidsMenu");
	
	@Autowired
	private GetNSKidsMenuService getNSKidsMenuService;
	
	public Object getNSKidsMenu(HttpServletRequest request, HttpServletResponse response , String szParam, String szStat) throws Exception
	{
		// 프로세스 정보
		...
		GetNSKidsMenuRequestVO requestVO = null;
		...
		try
		{
			requestVO	= new GetNSKidsMenuRequestVO(szParam);
		...
		try
		{
			...
			resultVO = this.getNSKidsMenuService.getNSKidsMenu(requestVO);
		}
		catch(ImcsException ie)
		{
			if(resultVO == null)		resultVO = new GetNSKidsMenuResultVO();
		}
		catch(Exception e)
		{
			if(resultVO == null)		resultVO = new GetNSKidsMenuResultVO();
