...
@Controller
public class GetNSMnuListDtlController {
	private Log imcsLogger		= LogFactory.getLog("API_Common_getNSMnuListDtl");
	private Log statLogger		= LogFactory.getLog("TLO_getNSMnuListDtl");

	@Autowired
	private GetNSMnuListDtlService getNSMnuListDtlService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	public Object getNSMnuListDtl(HttpServletRequest request, HttpServletResponse response , String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSMnuListDtlResultVO resultVO		= new GetNSMnuListDtlResultVO();
		GetNSMnuListDtlRequestVO requestVO		= null;
		String saId		= "";
		String stbMac	= "";
		
		// 변수 초기화
		try {
			requestVO	= new GetNSMnuListDtlRequestVO(szParam);
		...
		try {
			...
			resultVO	= this.getNSMnuListDtlService.getNSMnuListDtl(requestVO);
		} catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSMnuListDtlResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSMnuListDtlResultVO(); 
		} finally {
			// 결과 로그 출력
