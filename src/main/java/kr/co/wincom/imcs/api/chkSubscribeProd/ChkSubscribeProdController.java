...
@Controller
public class ChkSubscribeProdController {
	private Log imcsLogger	= LogFactory.getLog("API_Common_chkSubscribeProd");
	private Log statLogger	= LogFactory.getLog("TLO_chkSubscribeProd");
	
	@Autowired
	private ChkSubscribeProdService ChkSubscribeProdService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	public Object chkSubscribeProd(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		ChkSubscribeProdRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new ChkSubscribeProdRequestVO(szParam);
		...
		try {
			...
			resultVO = this.ChkSubscribeProdService.chkSubscribeProd(requestVO);
		} catch(ImcsException ie) {
			if( ie.getList() != null )	resultVO	= (ChkSubscribeProdResultVO) ie.getList();
			else						resultVO = new ChkSubscribeProdResultVO();
			
			resultVO.setFlag(ie.getFlag());
			resultVO.setErrCode(ie.getErrorCode());
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new ChkSubscribeProdResultVO();
			resultVO.setFlag("1");
