...
@Controller
public class RmNSAlertController {
	private Log imcsLogger		= LogFactory.getLog("API_Common_rmNSAlert");
	private Log statLogger		= LogFactory.getLog("TLO_rmNSAlert");

	@Autowired
	private RmNSAlertService		rmNSAlertService;
	 ...
	 * 알림받기 삭제 (lgvod662.pc)
	 */
	public Object rmNSAlert(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception{
		// 프로세스 정보
		...
		RmNSAlertResultVO resultVO		= new RmNSAlertResultVO();
		RmNSAlertRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		
		// 변수 초기화
		try {
			requestVO	= new RmNSAlertRequestVO(szParam);
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_PRO_ID662) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.API_PRO_ID662 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO	= this.rmNSAlertService.rmNSAlert(requestVO);
		} catch(ImcsException ie) {
			if( ie.getList() != null )	resultVO = (RmNSAlertResultVO) ie.getList();
			else						resultVO = new RmNSAlertResultVO();
			
			resultVO.setFlag(ie.getFlag());
			resultVO.setErrMsg(ie.getMessage());
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new RmNSAlertResultVO(); 
			resultVO.setFlag("1");
