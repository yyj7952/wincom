...
@Controller
public class GetNSPageListController {
	private Log imcsLogger		= LogFactory.getLog("API_Common_getNSPageList");
	private Log statLogger		= LogFactory.getLog("TLO_getNSPageList");

	@Autowired
	private GetNSPageListService		getNSPageListService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	public Object getNSPageList(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSPageListResultVO resultVO		= new GetNSPageListResultVO();
		GetNSPageListRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		
		// 변수 초기화
		try {
			requestVO	= new GetNSPageListRequestVO(szParam);
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.NSAPI_PRO_ID995) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.NSAPI_PRO_ID995 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO	= getNSPageListService.getNSPageList(requestVO);
		} catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSPageListResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSPageListResultVO();
		} finally {
			// 결과 로그 출력
