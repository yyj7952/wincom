...
@Controller
public class GetNSFavorListController {
	private Log imcsLogger		= LogFactory.getLog("API_Common_getNSFavorList");
	private Log statLogger		= LogFactory.getLog("TLO_getNSFavorList");
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	@Autowired
	private GetNSFavorListService		getNSFavorListService;

	public Object getNSFavorList(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSFavorListResultVO resultVO		= new GetNSFavorListResultVO();
		GetNSFavorListRequestVO requestVO	= null;
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSFavorListRequestVO(szParam);
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_PRO_ID339) + "] sts[start] rcv[" + szParam + "]";
		...
		try {
			...
			resultVO	= this.getNSFavorListService.getNSFavorList(requestVO);
		} catch(ImcsException ie) {
			if(resultVO == null)		resultVO = new GetNSFavorListResultVO();
		} catch(Exception e) {
			if(resultVO == null)		resultVO = new GetNSFavorListResultVO();
		} finally {
			// 결과 로그 출력
