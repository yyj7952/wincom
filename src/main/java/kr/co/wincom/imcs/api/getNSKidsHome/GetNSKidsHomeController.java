...
@Controller
public class GetNSKidsHomeController
{
	private final static String API_LOG_NAME = "000/getNSKidsHome";
	
	private Log imcsLogger		= LogFactory.getLog("API_Common_getNSKidsHome");
	private Log statLogger		= LogFactory.getLog("TLO_getNSKidsHome");
	
	@Autowired
	private GetNSKidsHomeService getNSKidsHomeService;
	
	public Object getNSKidsHome(HttpServletRequest request, HttpServletResponse response , String szParam, String szStat) throws Exception
	{
		// 프로세스 정보
		...
		GetNSKidsHomeRequestVO requestVO = null;
		...
		try
		{
			requestVO	= new GetNSKidsHomeRequestVO(szParam);
		...
		try
		{
			...
			resultVO = this.getNSKidsHomeService.getNSKidsHome(requestVO);
		}
		catch(ImcsException ie)
		{
			if(resultVO == null)		resultVO = new GetNSKidsHomeResultVO();
		}
		catch(Exception e)
		{
			if(resultVO == null)		resultVO = new GetNSKidsHomeResultVO();
