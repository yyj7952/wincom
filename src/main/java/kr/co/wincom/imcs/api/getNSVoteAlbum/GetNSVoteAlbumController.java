...
@Controller
public class GetNSVoteAlbumController {
	private Log imcsLogger	= LogFactory.getLog("API_Common_getNSVoteAlbum");
	private Log statLogger	= LogFactory.getLog("TLO_getNSVoteAlbum");
	
	@Autowired
	private GetNSVoteAlbumService GetNSVoteAlbumService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	public Object getNSVoteAlbum(HttpServletRequest request, HttpServletResponse response
			, String szParam, String szStat) throws Exception
	{
		...
		GetNSVoteAlbumResultVO resultVO	= new GetNSVoteAlbumResultVO();
		GetNSVoteAlbumRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSVoteAlbumRequestVO(szParam);
			...
			// 인입로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.NSAPI_PRO_ID045) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.NSAPI_PRO_ID045 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
		    ...
		    resultVO = this.GetNSVoteAlbumService.getNSVoteAlbum(requestVO);
		}  catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSVoteAlbumResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSVoteAlbumResultVO();
		} finally {
			// 결과 로그 출력
