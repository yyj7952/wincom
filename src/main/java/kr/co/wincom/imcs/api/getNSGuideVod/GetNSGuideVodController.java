...
@Controller
public class GetNSGuideVodController {
	private Log imcsLogger		= LogFactory.getLog("API_Common_getNSGuideVod");
	private Log statLogger		= LogFactory.getLog("TLO_getNSGuideVod");

	@Autowired
	private GetNSGuideVodService	getNSGuideVodService;
	 ...
	 */
	public Object getNSGuideVod(HttpServletRequest request, HttpServletResponse response , String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSGuideVodResultVO resultVO		= new GetNSGuideVodResultVO();
		GetNSGuideVodRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		
		// 변수 초기화
		try {
			requestVO	= new GetNSGuideVodRequestVO(szParam);
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_PRO_ID569) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.API_PRO_ID569 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO	= this.getNSGuideVodService.getNSGuideVod(requestVO);
		} catch(ImcsException ie) {
			if(resultVO == null)		resultVO = new GetNSGuideVodResultVO();
		} catch(Exception e) {
			if(resultVO == null)		resultVO = new GetNSGuideVodResultVO();
		} finally {
			// 결과 로그 출력
