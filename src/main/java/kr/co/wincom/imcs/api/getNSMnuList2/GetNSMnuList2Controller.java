...
@Controller
public class GetNSMnuList2Controller {
	private Log imcsLogger	= LogFactory.getLog("API_Common_getNSMnuList2");
	private Log statLogger	= LogFactory.getLog("TLO_getNSMnuList2");

	@Autowired
	private GetNSMnuList2Service GetNSMnuList2Service;
	
	public Object getNSMnuList2(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSMnuList2ResultVO resultVO	= new GetNSMnuList2ResultVO();
		GetNSMnuList2RequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSMnuList2RequestVO(szParam);
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.NSAPI_PRO_ID900) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.NSAPI_PRO_ID900 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO	= this.GetNSMnuList2Service.getNSMnuList2(requestVO);
		}  catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSMnuList2ResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSMnuList2ResultVO();
		} finally {
			// 결과 로그 출력
