...
@Controller
public class GetNSMakeNodeListController {

	private Log imcsLogger	= LogFactory.getLog("API_Common_getNSMakeNodeList");
	private Log statLogger	= LogFactory.getLog("TLO_getNSMakeNodeList");

	@Autowired
	private GetNSMakeNodeListService GetNSMakeNodeListService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	public Object getNSMakeNodeList(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSMakeNodeListResultVO resultVO		= new GetNSMakeNodeListResultVO();
		GetNSMakeNodeListRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSMakeNodeListRequestVO(szParam, request.getQueryString());
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.NSAPI_PRO_ID050) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.NSAPI_PRO_ID050 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO	= this.GetNSMakeNodeListService.getNSMakeNodeList(requestVO);
		} catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSMakeNodeListResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSMakeNodeListResultVO();
		} finally {
			// 결과 로그 출력
