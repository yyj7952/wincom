...
@Controller
public class GetNSContDtlController {
	private Log imcsLogger		= LogFactory.getLog("API_Common_getNSContDtl");
	private Log statLogger		= LogFactory.getLog("TLO_getNSContDtl");

	@Autowired
	private GetNSContDtlService GetNSContDtlService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	public Object getNSContDtl(HttpServletRequest request, HttpServletResponse response , String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSContDtlResultVO resultVO		= new GetNSContDtlResultVO();
		GetNSContDtlRequestVO requestVO		= null;
		String saId		= "";
		String stbMac	= "";
		
		// 변수 초기화
		try {
			requestVO	= new GetNSContDtlRequestVO(szParam);
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_PRO_ID996) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.API_PRO_ID996 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO	= this.GetNSContDtlService.getNSContDtl(requestVO);
		} catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSContDtlResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSContDtlResultVO(); 
		} finally {
			// 결과 로그 출력
