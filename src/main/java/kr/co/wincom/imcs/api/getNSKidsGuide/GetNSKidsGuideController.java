...
@Controller
public class GetNSKidsGuideController
{
	private final static String API_LOG_NAME = "000/getNSKidsGuide";
	
	private Log imcsLogger		= LogFactory.getLog("API_Common_getNSKidsGuide");
	private Log statLogger		= LogFactory.getLog("TLO_getNSKidsGuide");
	
	@Autowired
	private GetNSKidsGuideService getNSKidsGuideService;
	
	public Object getNSKidsGuide(HttpServletRequest request, HttpServletResponse response , String szParam, String szStat) throws Exception
	{
		// 프로세스 정보
		...
		GetNSKidsGuideRequestVO requestVO = null;
		...
		try
		{
			requestVO	= new GetNSKidsGuideRequestVO(szParam);
		...
		catch(Exception e)
		{
			...
			return "1|INPUT PARAM ERROR||0||\f";
		}
		...
		try
		{
			...
			resultVO = this.getNSKidsGuideService.getNSKidsGuide(requestVO);
		}
		catch(ImcsException ie)
		{
			if(resultVO == null)		resultVO = new GetNSKidsGuideResultVO();
		}
		catch(Exception e)
		{
			if(resultVO == null)		resultVO = new GetNSKidsGuideResultVO();
