...
@Controller
public class GetNSCHRatingsController {
	private Log imcsLogger	= LogFactory.getLog("API_Common_getNSCHRatings");
	private Log statLogger	= LogFactory.getLog("TLO_getNSCHRatings");

	@Autowired
	private GetNSCHRatingsService GetNSCHRatingsService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	public Object getNSCHRatings(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSCHRatingsResultVO resultVO		= new GetNSCHRatingsResultVO();
		GetNSCHRatingsRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSCHRatingsRequestVO(szParam);
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_PRO_ID982) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.API_PRO_ID982 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
		    ...
		    resultVO		= this.GetNSCHRatingsService.getNSCHRatings(requestVO);
		} catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSCHRatingsResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSCHRatingsResultVO();
		} finally {
			// 결과 로그 출력
