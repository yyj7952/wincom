...
@Controller
public class GetNSHighLightController {
	
	private Log imcsLogger		= LogFactory.getLog("API_Common_getNSHighLight");
	private Log statLogger		= LogFactory.getLog("TLO_getNSHighLight");

	@Autowired
	private GetNSHighLightService getNSHighLightService;
	
	/**
	 *
	 */
	public Object getNSHighLight(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSHighLightResultVO resultVO		= new GetNSHighLightResultVO();
		GetNSHighLightRequestVO requestVO	= null;
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSHighLightRequestVO(szParam);
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_PRO_ID990) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.API_PRO_ID990 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO = this.getNSHighLightService.getNSHighLight(requestVO);			
		} catch(ImcsException ie) {
			if (resultVO == null)		resultVO = new GetNSHighLightResultVO();
		} catch(Exception e) {
			if (resultVO == null)		resultVO = new GetNSHighLightResultVO();
		} finally {
			// 결과 로그 출력
