...
@Controller
public class RmNSCHFavorController {
	private Log imcsLogger	= LogFactory.getLog("API_Common_rmNSCHFavor");
	private Log statLogger	= LogFactory.getLog("TLO_rmNSCHFavor");

	@Autowired
	private RmNSCHFavorService RmNSCHFavorService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	public Object rmNSCHFavor(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		RmNSCHFavorRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new RmNSCHFavorRequestVO(szParam);
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_PRO_ID347) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.API_PRO_ID347 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO	= this.RmNSCHFavorService.rmNSCHFavor(requestVO);
		} catch(ImcsException ie) {
			if( ie.getList() != null )	resultVO = (RmNSCHFavorResultVO) ie.getList();
			else						resultVO = new RmNSCHFavorResultVO();
			resultVO.setFlag(ie.getFlag());
			resultVO.setErrMsg(ie.getMessage());
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new RmNSCHFavorResultVO();
			resultVO.setFlag("1");
