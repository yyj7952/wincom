...
@Controller
public class GetNSMnuListController {
	private Log imcsLogger	= LogFactory.getLog("API_Common_getNSMnuList");
	private Log statLogger	= LogFactory.getLog("TLO_getNSMnuList");

	@Autowired
	private GetNSMnuListService GetNSMnuListService;
	
	public Object getNSMnuList(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSMnuListResultVO resultVO	= new GetNSMnuListResultVO();
		GetNSMnuListRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSMnuListRequestVO(szParam);
		...
		try {
			...
			resultVO	= this.GetNSMnuListService.getNSMnuList(requestVO);
		}  catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSMnuListResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSMnuListResultVO();
		} finally {
			// 결과 로그 출력
