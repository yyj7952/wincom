...
@Controller
public class GetNSChList2Controller {
	private Log imcsLogger	= LogFactory.getLog("API_Common_getNSChList2");
	private Log statLogger	= LogFactory.getLog("TLO_getNSChList2");

	@Autowired
	private GetNSChList2Service GetNSChList2Service;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";
	public Object getNSChList2(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSChList2ResultVO resultVO	= new GetNSChList2ResultVO();
		GetNSChList2RequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSChList2RequestVO(szParam);
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.NSAPI_PRO_ID020) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.NSAPI_PRO_ID020 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
			imcsLog.serviceLog(msg, methodName, methodLine);
			
			String Header = "1|INPUT PARAM ERROR|";
			resultVO.setResultHeader(Header);
			return resultVO;
		...
		try {
			...
			resultVO	= this.GetNSChList2Service.getNSChList2(requestVO);
		}  catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSChList2ResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSChList2ResultVO();
		} finally {
			// 결과 로그 출력
