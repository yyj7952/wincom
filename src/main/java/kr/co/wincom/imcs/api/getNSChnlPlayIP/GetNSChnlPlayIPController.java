...
@Controller
public class GetNSChnlPlayIPController {

	private Log imcsLogger	= LogFactory.getLog("API_Common_getNSChnlPlayIP");
	private Log statLogger	= LogFactory.getLog("TLO_getNSChnlPlayIP");

	@Autowired
	private GetNSChnlPlayIPService GetNSChnlPlayIPService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	public Object getNSChnlPlayIP(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSChnlPlayIPResultVO resultVO		= new GetNSChnlPlayIPResultVO();
		GetNSChnlPlayIPRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSChnlPlayIPRequestVO(szParam);
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.NSAPI_PRO_ID060) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.NSAPI_PRO_ID060 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO	= this.GetNSChnlPlayIPService.getNSChnlPlayIP(requestVO);
		} catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSChnlPlayIPResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSChnlPlayIPResultVO();
		} finally {
			// 결과 로그 출력
