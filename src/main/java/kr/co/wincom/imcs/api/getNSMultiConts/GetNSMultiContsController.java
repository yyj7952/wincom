...
@Controller
public class GetNSMultiContsController {
	private Log imcsLogger		= LogFactory.getLog("API_Common_getNSMultiConts");
	private Log statLogger		= LogFactory.getLog("TLO_getNSMultiConts");

	@Autowired
	private GetNSMultiContsService		getNSMultiContsService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	public Object getNSMultiConts(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSMultiContsResultVO resultVO		= new GetNSMultiContsResultVO();
		GetNSMultiContsRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSMultiContsRequestVO(szParam);
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.NSAPI_PRO_ID012) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.API_PRO_ID339 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO	= getNSMultiContsService.getNSMultiConts(requestVO);
		} catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSMultiContsResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSMultiContsResultVO();
		} finally {
			// 결과 로그 출력
