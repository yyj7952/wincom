...
import kr.co.wincom.imcs.api.getNSKidsMenu.GetNSKidsMenuResultVO;
...
@Controller
public class GetNSGoodsListController
{
	private final static String API_LOG_NAME = "000/getNSGoodsList";
	
	private Log imcsLogger		= LogFactory.getLog("API_Common_getNSGoodsList");
	private Log statLogger		= LogFactory.getLog("TLO_getNSGoodsList");
	
	@Autowired
	private GetNSGoodsListService getNSGoodsListService;
	
	public Object getNSGoodsList(HttpServletRequest request, HttpServletResponse response , String szParam, String szStat) throws Exception
	{
		// 프로세스 정보
		...
		GetNSGoodsListRequestVO requestVO = null;
		...
		try
		{
			requestVO	= new GetNSGoodsListRequestVO(szParam);
		...
		try
		{
			...
			resultVO = this.getNSGoodsListService.getNSGoodsList(requestVO);
		}
		catch(ImcsException ie)
		{
			if(resultVO == null)		resultVO = new GetNSGoodsListResultVO();
		}
		catch(Exception e)
		{
			if(resultVO == null)		resultVO = new GetNSGoodsListResultVO();
