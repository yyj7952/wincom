...
@Controller
public class GetNSChListController {
	private Log imcsLogger	= LogFactory.getLog("API_Common_getNSChList");
	private Log statLogger	= LogFactory.getLog("TLO_getNSChList");

	@Autowired
	private GetNSChListService GetNSChListService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";
	public Object getNSChList(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSChListResultVO resultVO	= new GetNSChListResultVO();
		GetNSChListRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSChListRequestVO(szParam);
		...
		try {
			...
			resultVO	= this.GetNSChListService.getNSChList(requestVO);
		}  catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSChListResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSChListResultVO();
		} finally {
			// 결과 로그 출력
