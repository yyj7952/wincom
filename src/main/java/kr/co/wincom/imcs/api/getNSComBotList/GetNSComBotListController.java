...
@Controller
public class GetNSComBotListController {
	private Log imcsLogger	= LogFactory.getLog("API_Common_getNSComBotList");
	private Log statLogger	= LogFactory.getLog("TLO_getNSComBotList");

	@Autowired
	private GetNSComBotListService GetNSComBotListService;

	public Object getNSComBotList(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSComBotListResultVO resultVO		= new GetNSComBotListResultVO();
		GetNSComBotListRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSComBotListRequestVO(szParam);
			...
			// 인입로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.NSAPI_GETNSCOMBOTLIST) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.NSAPI_GETNSCOMBOTLIST + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO = this.GetNSComBotListService.getNSComBotList(requestVO);
		} catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSComBotListResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSComBotListResultVO();
		} finally {
			// 결과 로그 출력
