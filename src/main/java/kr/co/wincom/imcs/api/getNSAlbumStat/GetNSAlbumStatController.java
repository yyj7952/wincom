...
@Controller
public class GetNSAlbumStatController
{
	private final static String API_LOG_NAME = "000/getNSAlbumStat";
	
	private Log imcsLogger		= LogFactory.getLog("API_Common_getNSAlbumStat");
	private Log statLogger		= LogFactory.getLog("TLO_getNSAlbumStat");
	
	@Autowired
	private GetNSAlbumStatService getNSAlbumStatService;
	
	public Object getNSAlbumStat(HttpServletRequest request, HttpServletResponse response , String szParam, String szStat) throws Exception
	{
		// 프로세스 정보
		...
		GetNSAlbumStatRequestVO requestVO = null;
		...
		try
		{
			requestVO	= new GetNSAlbumStatRequestVO(szParam);
		...
		try
		{
			...
			resultVO = this.getNSAlbumStatService.getNSAlbumStat(requestVO);
		}
		catch(ImcsException ie)
		{
			if(resultVO == null)		resultVO = new GetNSAlbumStatResultVO();
		}
		catch(Exception e)
		{
			if(resultVO == null)		resultVO = new GetNSAlbumStatResultVO();
