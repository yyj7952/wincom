...
@Controller
public class GetNSContReviewController {
	private Log imcsLogger	= LogFactory.getLog("API_Common_getNSContReview");
	private Log statLogger	= LogFactory.getLog("TLO_getNSContReview");
	
	@Autowired
	private GetNSContReviewService GetNSContReviewService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	public Object getNSContReview(HttpServletRequest request, HttpServletResponse response
			, String szParam, String szStat) throws Exception
	{
		...
		GetNSContReviewResultVO resultVO	= new GetNSContReviewResultVO();
		GetNSContReviewRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSContReviewRequestVO(szParam);
			...
			// 인입로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.NSAPI_PRO_ID410) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.NSAPI_PRO_ID410 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
		    ...
		    resultVO = this.GetNSContReviewService.getNSContReview(requestVO);
		}  catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSContReviewResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSContReviewResultVO();
		} finally {
			// 결과 로그 출력
