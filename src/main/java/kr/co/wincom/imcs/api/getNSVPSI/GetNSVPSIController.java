...
@Controller
public class GetNSVPSIController {

	private Log imcsLogger	= LogFactory.getLog("API_Common_getNSVPSI");
	private Log statLogger	= LogFactory.getLog("TLO_getNSVPSI");

	@Autowired
	private GetNSVPSIService GetNSVPSIService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	public Object getNSVPSI(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSVPSIResultVO resultVO		= new GetNSVPSIResultVO();
		GetNSVPSIRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSVPSIRequestVO(szParam);
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_PRO_ID031) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.API_PRO_ID031 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO	= this.GetNSVPSIService.getNSVPSI(requestVO);
		} catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSVPSIResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSVPSIResultVO();
		} finally {
			// 결과 로그 출력
