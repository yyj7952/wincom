...
@Controller
public class GetNSCustInfoController {
	private Log imcsLogger =	LogFactory.getLog("API_Common_getNSCustInfo");
	private Log statLogger =	LogFactory.getLog("TLO_getNSCustInfo");
	
	@Autowired
	private GetNSCustInfoService getNSCustInfoService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";
	
	public Object getNSCustInfo(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSCustInfoRequestVO requestVO		= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSCustInfoRequestVO(szParam);
			...
			// 입입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.NSAPI_PRO_ID001) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.NSAPI_PRO_ID001 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO	= this.getNSCustInfoService.getNSCustInfo(requestVO);
		} catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSCustInfoResultVO();
