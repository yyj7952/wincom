...
@Controller
public class SetNSPointController {
		
	private Log imcsLogger = LogFactory.getLog("API_Common_setNSPoint");
	private Log statLogger = LogFactory.getLog("TLO_setNSPoint");
	
	@Autowired
	private SetNSPointService SetNSPointService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";
	
	public Object setNSPoint(HttpServletRequest request, HttpServletResponse response
			, String szParam, String szStat) throws Exception
	{
		...
		SetNSPointRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO 	= new SetNSPointRequestVO(szParam);
			...
			// 입입로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_PRO_ID609) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.API_PRO_ID609 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO = this.SetNSPointService.setNSPoint(requestVO);
		}  catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new SetNSPointResultVO();
