...
@Controller
public class GetNSCatBillInfoController {
	private Log imcsLogger		= LogFactory.getLog("API_Common_getNSCatBillInfo");
	private Log statLogger		= LogFactory.getLog("TLO_getNSCatBillInfo");
	
	@Autowired
	private GetNSCatBillInfoService getNSCatBillInfoService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";
	
	public Object getNSCatBillInfo(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSCatBillInfoResultVO resultVO		= new GetNSCatBillInfoResultVO();
		GetNSCatBillInfoRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSCatBillInfoRequestVO(szParam);
		...
		try {
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.NSAPI_PRO_ID011) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			requestVO.setPid(pId);
			resultVO	= this.getNSCatBillInfoService.getNSCatBillInfo(requestVO);
		} catch(ImcsException ie) {
			if(resultVO == null)		resultVO = new GetNSCatBillInfoResultVO();
		} catch(Exception e) {
			if(resultVO == null)		resultVO = new GetNSCatBillInfoResultVO();
		} finally {
			// 결과 로그 출력
