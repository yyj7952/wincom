...
@Controller
public class AddNSCHFavorController {
		
	private Log imcsLogger = LogFactory.getLog("API_Common_addNSCHFavor");
	private Log statLogger = LogFactory.getLog("TLO_addNSCHFavor");
	
	@Autowired
	private AddNSCHFavorService AddNSCHFavorService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";
		
	public Object addNSCHFavor(HttpServletRequest request, HttpServletResponse response
			, String szParam, String szStat) throws Exception
	{
		...
		AddNSCHFavorRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new AddNSCHFavorRequestVO(szParam);
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_PRO_ID377) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.API_PRO_ID377 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO	= this.AddNSCHFavorService.addNSCHFavor(requestVO);
		} catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new AddNSCHFavorResultVO();
			...
			resultVO.setFlag("1");
			resultVO.setErrCode("선호채널 등록이 실패하였습니다.");
			resultVO.setErrMsg("03");
		} finally {
			// 결과 로그 출력
