...
@Controller
public class RmNSPresentController {
	private Log imcsLogger =	LogFactory.getLog("API_Common_rmNSPresent");
	private Log statLogger =	LogFactory.getLog("TLO_rmNSPresent");
	
	@Autowired
	private RmNSPresentService RmNSPresentService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";
	
	public Object rmNSPresent(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception	{
		// 프로세스 정보
		...
		RmNSPresentRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new RmNSPresentRequestVO(szParam);
			...
			// 입입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_PRO_ID313) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.API_PRO_ID313 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO	= this.RmNSPresentService.rmNSPresent(requestVO);
		} catch(ImcsException ie) {
			if( ie.getList() != null )	resultVO = (RmNSPresentResultVO) ie.getList();
			else						resultVO = new RmNSPresentResultVO();
			resultVO.setFlag(ie.getFlag());
			resultVO.setErrMsg(ie.getMessage());
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new RmNSPresentResultVO();
			resultVO.setFlag("1");
