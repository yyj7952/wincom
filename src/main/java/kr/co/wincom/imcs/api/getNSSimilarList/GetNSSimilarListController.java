...
@Controller
public class GetNSSimilarListController {
	private Log imcsLogger		= LogFactory.getLog("API_Common_getNSSimilarList");
	private Log statLogger		= LogFactory.getLog("TLO_getNSSimilarList");

	@Autowired
	private GetNSSimilarListService GetNSSimilarListService;
	 ...
	 * 비슷한 영화(왓챠) 정보 조회
	 */
	public Object getNSSimilarList(HttpServletRequest request, HttpServletResponse response, String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSSimilarListResultVO resultVO	= new GetNSSimilarListResultVO();
		GetNSSimilarListRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new GetNSSimilarListRequestVO(szParam);
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_PRO_ID985) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.API_PRO_ID985 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO	= this.GetNSSimilarListService.getNSSimilarList(requestVO);
		} catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSSimilarListResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSSimilarListResultVO();
		} finally {
			// 결과 로그 출력
