...
@Controller
public class RmNSWatchHisController {
	private Log imcsLogger	= LogFactory.getLog("API_Common_rmNSWatchHis");
	private Log statLogger	= LogFactory.getLog("TLO_rmNSWatchHis");

	@Autowired
	private RmNSWatchHisService RmNSWatchHisService;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	public Object rmNSWatchHis(HttpServletRequest request, HttpServletResponse response , String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		RmNSWatchHisResultVO resultVO	= new RmNSWatchHisResultVO();
		RmNSWatchHisRequestVO requestVO	= null;
		String saId		= "";
		String stbMac	= "";
		...
		// 변수 초기화
		try {
			requestVO	= new RmNSWatchHisRequestVO(szParam);
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.API_PRO_ID348) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.API_PRO_ID348 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO	= this.RmNSWatchHisService.rmNSWatchHis(requestVO);
		} catch(ImcsException ie) {
			if( ie.getList() != null )	resultVO = (RmNSWatchHisResultVO) ie.getList();
			else						resultVO = new RmNSWatchHisResultVO();
			resultVO.setFlag(ie.getFlag());
			resultVO.setErrMsg(ie.getMessage());
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new RmNSWatchHisResultVO();
			resultVO.setFlag("1");
