...
@Controller
public class GetNSMnuListDtl2Controller {
	private Log imcsLogger		= LogFactory.getLog("API_Common_getNSMnuListDtl2");
	private Log statLogger		= LogFactory.getLog("TLO_getNSMnuListDtl2");

	@Autowired
	private GetNSMnuListDtl2Service getNSMnuListDtl2Service;
	
	//protected static String saId	= "";
	//protected static String stbMac	= "";

	public Object getNSMnuListDtl2(HttpServletRequest request, HttpServletResponse response , String szParam, String szStat) throws Exception {
		// 프로세스 정보
		...
		GetNSMnuListDtl2ResultVO resultVO		= new GetNSMnuListDtl2ResultVO();
		GetNSMnuListDtl2RequestVO requestVO		= null;
		String saId		= "";
		String stbMac	= "";
		
		// 변수 초기화
		try {
			requestVO	= new GetNSMnuListDtl2RequestVO(szParam);
			...
			// 인입 로그
			String szMsg = " svc[" + String.format("%-20s", ImcsConstants.NSAPI_PRO_ID910) + "] sts[start] rcv[" + szParam + "]";
			imcsLog.serviceLog(szMsg, methodName, methodLine);
			
			String msg = " svc[" + ImcsConstants.NSAPI_PRO_ID910 + "] sts[error] msg["+ImcsConstants.RCV_MSG1 +"]";
		...
		try {
			...
			resultVO	= this.getNSMnuListDtl2Service.getNSMnuListDtl2(requestVO);
		} catch(ImcsException ie) {
			if( resultVO == null )		resultVO = new GetNSMnuListDtl2ResultVO();
		} catch(Exception e) {
			if( resultVO == null )		resultVO = new GetNSMnuListDtl2ResultVO(); 
		} finally {
			// 결과 로그 출력
			...
			statVO.setGenreLarge(resultVO.getGenreLarge());
			statVO.setGenreMid(resultVO.getGenreMid());
			statVO.setCreateDate(resultVO.getCreateDate());
